...
const AboutYourSelfPageNew = (props) => {
  ...
  */
  const validate = (key) => {
    let fields = key === 'all' ? ['email', 'password'] : [key];
    let errorsData: any = errors ? errors : {};
    fields.forEach(field => {
      let loginFormelements = loginForm.current;
      if (!loginFormelements[field] || loginFormelements[field].value.trim() === '') {
        errorsData[field] = (field === 'email' ? 'Email ' : 'Password ') + 'is required';
      } else {
        ...
        <form ref={loginForm} >
          <div className="about-yourself">
            ...
            <div className="input-form-fields">
              <div>
                <TextField type='text' name='email' placeholder='Enter Your Email Here'
                  autoComplete='off' onBlur={() => { validate('email'); }} error={errors && errors.email}
                  helperText={errors && errors.email ? "The First Name is required" : ""} />
                {/* <Typography >{errors && errors.email}</Typography> */}
              </div>
